	...
	<c:choose>
		<c:when test="${!empty sessionScope.user.userName}">
			<c:choose>
				...
				You are not authorized to be here. <a
						href="http://localhost:8080/HotelBooking/">Click Here to go to
						Home</a>
				</c:when>
				<c:when test="${sessionScope.user.userRole =='affiliate'}">
					...
					<h3>
						Welcome <font color="GREEN">${sessionScope.user.userName} </font>
						!
					</h3>
